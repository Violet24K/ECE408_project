...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    */
    ...
    #define sm3d_tree(i2, i1, i0) SM[i2 * TILE_WIDTH * C + i1 * C + i0]
    ...
    if (h_out < H_out && w_out < W_out)
    {
        ...
        for (int p = 0; p < K; p++)
        {
            for (int q = 0; q < K; q++)
            {
                result += x4d(b_out, c_in, h_out + p, w_out + q) * k4d(m_out, c_in, p, q);
            }
        }
        sm3d_tree(ty, tx, c_in) = result;
        ...
        for (int stride = 1; stride < C; stride *= 2)
        {
            ...
            {
                    sm3d_tree(ty, tx, tz) += sm3d_tree(ty, tx, tz + stride);
        ...
        if (tz == 0)
        {
            y4d(b_out, m_out, h_out, w_out) = sm3d_tree(ty, tx, 0);
        }
