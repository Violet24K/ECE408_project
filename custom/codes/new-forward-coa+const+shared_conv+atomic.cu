...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    */
    ...
    int TILE_leftup_h = TILE_WIDTH * (bz/numblock_eachrow);
    int TILE_leftup_w = TILE_WIDTH * (bz%numblock_eachrow);
    ...
    for(int i = ty; i < shared_width; i += TILE_WIDTH)
    {
        ...
        {
            if (TILE_leftup_h + i < H && TILE_leftup_w + j < W)
            {
                sm3d(c_in, i, j) = x4d(b_out, c_in, TILE_leftup_h + i, TILE_leftup_w + j);
            }
        ...
        float result = 0;
            for (int p = 0; p < K; p++)
            {
                for (int q = 0; q < K; q++)
                {
                    ...
                    result += sm3d(c_in, ty + p, tx + q) * k4d(m_out, c_in, p, q);
                }
            }
        atomicAdd((&y4d(b_out, m_out, h_out, w_out)), result);
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    conv_forward_kernel<<<dimGrid, dimBlock, C*(TILE_WIDTH + K - 1)*(TILE_WIDTH + K - 1)*sizeof(float)>>>(device_y, device_x, device_k, B, M, C, H, W, K);
}
